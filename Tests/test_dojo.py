...
class TestDojo(unittest.TestCase):
    ...
    def test_create_room_success(self):
        ...
        before_count = len(dojo.rooms)
        new_room = dojo.createRoom('office', ['German','Italy','Croatia'])
        after_count = len(dojo.rooms)
        self.assertEqual(before_count + len(['German','Italy','Croatia']), after_count )

    def test_room_is_does_not_exist_yet(self):
        """The room should not be in the system as yet"""
        new_room = dojo.createRoom('office', 'Germany')
        self.assertNotIn(new_room.name, dojo.rooms)


    def test_room_created_has_intended_attributes(self):
        """The room created should be either office or living space"""
        new_room = dojo.createRoom('office', 'Germany')
        self.assertTrue([new_room.type, new_room.name] == ['office', 'Germany'])


    def test_room_created_only_accepts_strings(self):
        """Input both inputs should be strings """
        self.assertRaises(TypeError, dojo.createRoom, 5, 2)
        self.assertRaises(TypeError, dojo.createRoom, 'office', 7)
        self.assertRaises(TypeError, dojo.createRoom, 8, '')

    # def test_room_created_is_instance_of_room(self):
    #     """The room should be instance of room"""
    #     new_room = dojo.createRoom('office', 'Germany')
    #     self.assertisInstance(new_room, person.Room)
